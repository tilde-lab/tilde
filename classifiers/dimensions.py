...
# accounts the 2d surface case (when the PW-based codes are forced to put vacuum in a "non-periodical" direction) and extends ASE object(s)
...
#ADDED_VACUUM1D_TOL = 12 # empirical criterion: if we have too much vacuum in 1d
ADDED_VACUUM3D_TOL = 12 # empirical criterion: if we have too much vacuum in 3d
...
def classify(tilde_obj):    
    if sum(tilde_obj.structures[-1].get_pbc()) == 3: # only those initially 3-periodic
        ...
        # 2. Second step: if we have too much vacuum
