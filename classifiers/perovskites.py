...
def classify(content_obj, tilde_obj):
    ...
    if not len(C_site): return content_obj
    
    
    # TODO: Goldschmidt tolerance factor
    ...
    if not AB: return content_obj
    D = float(C) / AB
    #print "->", D
    
    # todo: 2-component pseudo perovskites
    if content_obj['elements'][0] in ['W', 'Re'] and len(content_obj['elements']) == 2:
        if round(D) == 3: content_obj['tags'].append('perovskite')
        return content_obj
    
    if not 1.37 < D < 1.7: return content_obj
    
    content_obj['tags'].append('perovskite')
    if tilde_obj.structures[-1]['periodicity'] != 3: return content_obj # all below is for 3d case
        ...
        for i in tilde_obj.structures[-1]['atoms']:
            if i[0] == impurity_element:
                ...
                for k in range(len(dist_matrix)):
                    D_d = abs(dist_matrix[k+1]-dist_matrix[k]) / dist_matrix[k+1] # jump in distance change allows us to determine A- or B-site
                    # TODO: interstitials
                    if 0.12 < D_d < 0.28: # 
                        # A-site
