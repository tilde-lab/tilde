...
class Tilting():
    ...
    MAX_TILTING_DEGREE = 22.4                    # degree, this is for adjusting, may produce unphysical results
    ...
    def __init__(self, tilde_calc):
        ...
        self.virtual_atoms = tilde_calc.structures[-1].copy()

        # translate atoms around octahedra
        cellpar = cell_to_cellpar(tilde_calc.structures[-1].cell)
        centers = [[i.x, i.y, i.z] for i in tilde_calc.structures[-1] if i.symbol in Perovskite_Structure.B]
        centers = map(lambda u: sum(u)/len(u), zip(*centers))
        for n in range(3):
            if abs(centers[n]) < 0.4*cellpar[n]: centers[n] = 0
            else: centers[n] = int(math.copysign(1, centers[n]))
        shift_dirs = []
        ...
        for k, i in enumerate(tilde_calc.structures[-1]):
            if i.symbol in Perovskite_Structure.C:
                for dir in shift_dirs:
                    self.translate(k, tilde_calc.structures[-1].cell, dir, self.virtual_atoms)

        # extract octahedra
        for octahedron in self.get_octahedra(tilde_calc.structures[-1], tilde_calc.structures[-1].periodicity):
    ...
    def translate(self, num_of_atom, cell, components, reference):
        ''' for a given atom find a translation-equivalent one '''
        a_component, b_component, c_component = components
        reference.append(Atom(reference[num_of_atom].symbol,
        ...
        for k, i in enumerate(atoms):
            ...
            if i.symbol in Perovskite_Structure.B:
                for m, j in enumerate(self.virtual_atoms):
                    if j.symbol in Perovskite_Structure.C and self.virtual_atoms.get_distance(k, m) <= self.OCTAHEDRON_BOND_LENGTH_LIMIT:
                        found.append(m)
    ...
    def get_tiltplanes(self, sequence):
        ...
        filtered = filter(   lambda x: \
            abs(self.virtual_atoms[ x[0] ].z - self.virtual_atoms[ x[1] ].z) < self.OCTAHEDRON_ATOMS_Z_ORIENT_DIFFERENCE and \
            abs(self.virtual_atoms[ x[1] ].z - self.virtual_atoms[ x[2] ].z) < self.OCTAHEDRON_ATOMS_Z_ORIENT_DIFFERENCE and \
            abs(self.virtual_atoms[ x[2] ].z - self.virtual_atoms[ x[3] ].z) < self.OCTAHEDRON_ATOMS_Z_ORIENT_DIFFERENCE,   tilting_planes   )
    ...
    def get_tilting(self, oplane):
        ...
        beta = math.degrees(math.atan2(math.sqrt(self.virtual_atoms[tilt_z].x*self.virtual_atoms[tilt_z].x + self.virtual_atoms[tilt_z].y*self.virtual_atoms[tilt_z].y), self.virtual_atoms[tilt_z].z))
